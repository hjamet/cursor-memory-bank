...
server.tool('update_task', {
    ...
    comment: z.string().describe("COMMENTAIRE OBLIGATOIRE - Rédigez en français une explication détaillée de votre changement d'état. Ce commentaire est OBLIGATOIRE pour toute mise à jour de tâche. Instructions spécifiques par statut : (1) Si vous marquez la tâche comme IN_PROGRESS : le commentaire peut être laissé vide car aucune justification n'est nécessaire pour commencer le travail. (2) Si vous marquez la tâche comme BLOCKED : expliquez en détail pourquoi elle est bloquée, quels obstacles vous avez rencontrés, et ce que l'utilisateur peut faire pour la débloquer. (3) Si vous marquez la tâche comme REVIEW : expliquez ce que vous avez accompli, quels fichiers contiennent les résultats, quels tests manuels vous avez effectués pour vérifier le bon fonctionnement, et ce que l'utilisateur devrait vérifier pour valider votre travail. (4) Pour tout autre changement : expliquez clairement la raison du changement et son impact. Le commentaire doit être informatif, actionnable et aider l'utilisateur à comprendre vos actions et décisions."),
