...
7. **Mise à Jour Systématique de la Fiche de Projet Obsidian (Fin de Session)** :
   ...
   - **Règles d'or de mise à jour & Structure Tableau de Bord** :
     ...
     0bis. **Sauvegarde Pérenne des Plans d'Implémentation et Rapports Non Builts** : Si la session a produit des rapports d'exploration (`exploration_report_X.md`) ou un plan d'implémentation non encore exécuté par `/build`, les compiler en une note Obsidian structurée selon le format de plan d'implémentation du skill `build`, afin de sceller les arbitrages et de permettre une reprise sans friction dans un nouveau chat.
