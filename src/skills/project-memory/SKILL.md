...
2. **Protocole de Fin de Session en 4 Points (Obligatoire)** :
   ...
   2. **Synchronisation Totale de la Note Maîtresse & Sauvegarde des Artéfacts Non-Build** :
      - **Sauvegarde Obligatoire des Artéfacts Non-Build** : Tout plan d'implémentation (`implementation_plan.md`), rapport de cadrage ou note technique produit durant la session et non encore matérialisé dans le code par `/build` DOIT ÊTRE OBLIGATOIREMENT sauvegardé dans le coffre Obsidian sous : `notes/Plan d'Implémentation [NomProjet].md` (et indexé sous H1 de la note maîtresse).
      - **Feuille de Route Opérationnelle Synchronisée** : Mettre à jour en tête de note maîtresse la section `## 📋 Quelle est la feuille de route...` directement à partir de l'artéfact `task.md` (structure 2 niveaux, cases `[ ]`/`[x]`, liens cliquables, et actions Henri avec feux tricolores `🔴/🟡/🟢`).
      - **Garantie de Relais Sans Perte** : L'état du coffre doit permettre à n'importe quel nouvel agent de reprendre exactement là où la session s'est arrêtée, avec accès direct au plan d'implémentation et à la roadmap sans rien devoir reconstituer.
...
0. **Affichage Système & Lien Cliquable de la Note de Projet (MANDATOIRE DÈS LE DÉBUT)** :
   ...
   - **Initialisation & Enrichissement de `task.md`** : Au chargement du projet (`work` ou début de session), Antigravity **DOIT initialiser et enrichir l'artéfact `task.md`** avec une sélection compacte des dernières tâches accomplies (`- [x]`) et des prochaines tâches prioritaires (`- [ ]`) extraites de la feuille de route de la note maîtresse Obsidian.
...
3. **Session de Travail Pomodoro & Suivi de Rythme (Règle d'Or du Pomodoro Permanent)** :
   - **Zéro Travail sans Pomodoro** : Lancer systématiquement `work "<NomProjet>"`. Au tout début, afficher immédiatement le lien Markdown cliquable vers la note Obsidian et initialiser/compléter `task.md` avec la sélection compacte des tâches de la roadmap de la note maîtresse.
   - **Protocole de Clôture en 4 Points** : À l'échéance de la session Pomodoro (60 min) :
     ...
     2. Mettre à jour la Roadmap de la note maîtresse Obsidian DIRECTEMENT à partir de l'artéfact `task.md` de session (même format: `- [x]` / `- [ ]`, feux tricolores `🔴/🟡/🟢` pour les tâches Henri, et repliement de l'historique dans `<details><summary>✅ Cadrage Validé / Tâches Accomplies</summary>`). Il ne suffit pas de sauvegarder des plans d'implémentation : la mise à jour de la roadmap de la note maîtresse depuis `task.md` est le devoir n°1.
...
7. **Mise à Jour Systématique de la Fiche de Projet Obsidian (Fin de Session)** :
   ...
   - **Règles d'or de mise à jour & Structure Tableau de Bord** :
     0. **Roadmap en Tête (MANDATOIRE DÈS LE HAUT)** : Mettre à jour la feuille de route opérationnelle (`## 📋 Quelle est la feuille de route opérationnelle...`) placée au sommet de la note (sous l'en-tête visuel et l'index des sous-notes) **DIRECTEMENT à partir de l'artéfact `task.md` de session** (passerelle bidirectionnelle). Respecter scrupuleusement le même format natif (`- [x]` accompli, `- [ ]` restant, émojis évocateurs, liens cliquables, feux tricolores `🔴/🟡/🟢` pour les tâches Henri, et repliement de l'historique achevé dans `<details><summary>✅ Cadrage Validé / Tâches Accomplies</summary>`). Il ne suffit pas de sauvegarder des plans d'implémentation : la mise à jour de la roadmap de la note maîtresse depuis `task.md` est le devoir n°1.
