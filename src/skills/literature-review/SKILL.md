...
> [!IMPORTANT]
> **Abandon Formel du Format Canvas (.canvas) & Remplacement par le Tag `obsidian` :**
...
> - **Tag Obsidian Canonique** : L'ancien tag `canvas` est **remplacé formellement par le tag `obsidian`** dans le frontmatter des fiches d'articles et dans Zotero pour certifier l'intégration dans le coffre.
...
flowchart TD
    ...
    AG1 --> OUT1["📄 Note individuelle notes/<Année> <Titre>.md<br/>🎨 Infographie assets/infographie_<...>.jpg<br/>📑 Entrée canonique dans [[Revue de Littérature <Projet>]]<br/>🏷️ Tags Zotero 'reviewed' & 'obsidian'"]
...
1. **Détection & Sélection de la Cible** :
   ...
   - Filtrer les articles de la collection Zotero qui ne sont **pas encore revus** (tags `reviewed` ou `obsidian` absents dans Zotero ou article non encore inséré dans la note de synthèse).
...
4. **Rédaction de la Note Permanente Individuelle Obsidian (`notes/<Année> <Titre>.md`)** :
   ...
   - **Structure Intérieure Obligatoire** :
     1. **Frontmatter YAML Canonique (avec tag `obsidian`)** :
        ```yaml
        ---
        ...
        authors:
          - <Auteur 1>
          - <Auteur 2>
        venue: "<Nom de la Conférence ou Journal>"
        rank: "<CORE A* / CORE A / Q1>"
        year: <Année>
        doi: "<DOI ou identifiant arXiv>"
        url: "<URL source>"
        fit_score: fit-5 # (fit-1 à fit-5)
        tags:
          - paper-review
          - <NomDuProjet>
          - fit-5
          - obsidian
          - reviewed
        Image: "[[_attachments/infographie_<nom_court>.png]]"
        ---
        ```
     2. **Titre H1** : `# 📄 <Année> — <Titre Officiel de l'Article>`
     3. **Infographie Visuelle** : Insérée **immédiatement sous H1** : `![[infographie_<nom_court>.jpg]]`.
     4. **Bloc `> [!IMPORTANT]` (ZÉRO redite de métadonnées, ZÉRO abstract brut)** : Synthèse & Intuition centrale du papier (verrou scientifique, approche/solution, illustrations concrètes).
     5. **Section 1. Fiche d'Identité & Profil de la Venue** : Tableau des métadonnées (Auteurs, Affiliations, Publication, Rang CORE / IF, DOI/URL, Fit score, profil thématique).
     6. **Sections Analytiques Suivantes** :
        ...
        - *6. Références Croisées & Connexions Vault* : Connectée **UNIQUEMENT** à la note de revue de littérature `[[Revue de Littérature <NomDuProjet>]]` (zéro lien sortant superflu vers d'autres notes du coffre).
5. **Alimentation de la Note de Synthèse Markdown (`notes/Revue de Littérature [Nom du Projet].md`)** :
   ...
   - **Format Épuré Strict (Zéro Blabla, Zéro Tableau Redondant, Zéro BibTeX)** :
     Après le lien projet et le diagramme Mermaid initial, la note de synthèse présente exclusivement la suite séquentielle des fiches d'articles au format minimaliste :
     ```markdown
     # [[Nom de la note permanente du papier]]

     ![[infographie_du_papier.png]]
     ```
     (Aucun commentaire, aucun tableau récapitulatif lourd, aucun bloc de code BibTeX : Zotero est la source unique de vérité bibliographique).
6. **Synchronisation Zotero, Obsidian & AIVC** :
   - **Création et Synchronisation Zotero ↔ Obsidian** :
     * Mettre à jour l'item Zotero via `pyzotero` avec les tags `obsidian` (remplaçant l'ancien tag `canvas`), `reviewed`, `fit-X` et le chemin/URL du PDF.
...
2. **Audit & Élagage Actif des Papiers Faibles / Redondants** :
   - Examiner en priorité les articles **qui ne sont PAS encore analysés** dans la note de synthèse (non tagués `reviewed` ou `obsidian` dans Zotero).
   ...
   - *Sanctuarisation* : **Les articles déjà intégrés dans la note de synthèse Markdown `[[Revue de Littérature NomDuProjet]]` (tagués `reviewed` et `obsidian`) sont actés comme validés et ne doivent JAMAIS être supprimés.**
...
5. **Mise à Jour de la Cartographie & Synchronisation Obsidian** :
   - Mettre à jour les entrées dans la note de synthèse `notes/Revue de Littérature [Nom du Projet].md` (liée à `[[NomDuProjet]]`).
...
> **Tag d'Intégration Vault** : Le tag Zotero et Obsidian **`obsidian`** (qui remplace formellement l'ancien tag `canvas`) certifie qu'une note Markdown individuelle a été créée et intégrée dans le coffre.
...
4. **Structure Canonique d'Article dans la Synthèse (Format Épuré)** :
   ```markdown
   # [[Nom de la note du papier]]

   ![[Infographie du papier]]
   ```
   Strictement aucun blabla, aucun tableau de 30 lignes, aucun bloc BibTeX.
5. **Tag `obsidian` Remplaçant `canvas`** : Utiliser impérativement le tag `obsidian` (et non plus `canvas`) dans le frontmatter des fiches individuelles et dans Zotero.
6. **Connexion Unique des Notes Individuelles** : Les fiches individuelles sont connectées UNIQUEMENT à la note de revue de littérature `[[Revue de Littérature <NomDuProjet>]]` pour éviter le spam de backlinks.
7. **Création & Synchronisation Bilatérale Zotero ↔ Obsidian** : Enrichissement mutuel continu des fiches du coffre et de la base Zotero.
8. **Anti-Complaisance (Anti-Sycophancy)** : Rigueur médico-légale pure dans les notes de revue individuelles.
...
```markdown
---
Image: "[[_attachments/infographie_<nom_court_1>.png]]"
---
# 📚 Quels Papiers Fondateurs et Cadres Théoriques Ancrent le Projet <NomDuProjet> ?
...
mindmap
  root((🎲 <NomDuProjet>))
    Axe 1: <Thème 1>
      Papier A 2024 (Description courte)
      Papier B 2023 (Description courte)
    Axe 2: <Thème 2>
      Papier C 2024 (Description courte)
```

---

# [[<Année> <Titre de l'Article 1>]]

![[infographie_<nom_court_1>.png]]

# [[<Année> <Titre de l'Article 2>]]

![[infographie_<nom_court_2>.png]]
```
...
| **3. Fiche Permanente** | `notes/<Année> <Titre>.md` | Rédaction en Français de la fiche médico-légale individuelle avec infographie en tête, tag `obsidian` et lien unique vers la revue. |
| **4. Note de Synthèse Markdown** | `notes/Revue de Littérature [Nom du Projet].md` | Insertion de l'entrée épurée : `# [[Lien]]` + `![[Infographie]]`, sans blabla ni BibTeX. |
| **5. Synchronisation Zotero & Obsidian** | pyzotero / Vault | Enrichissement mutuel, tags `fit-X`, `obsidian` (remplaçant `canvas`) et `reviewed`, alignement des métadonnées, zéro export `.bib`. |
