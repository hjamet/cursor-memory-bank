...
Quand l'utilisateur tape `/pull`, agir en **autonomie totale** :
...
## 🔑 Quels Sont les Prérequis d'Accès et de Détection ?
...
## 🔄 Comment S'Enchaînent les 5 Phases d'Exécution ?

```mermaid
flowchart TD
    P1["📋 Phase 1 : Inventaire des PRs (label: valid)"] --> P2{"PRs éligibles trouvées ?"}
    P2 -->|Non| S1["🛑 Arrêt propre (aucune PR à merger)"]
    P2 -->|Oui| P3["🔀 Phase 2 : Merge Autonome & Conflits"]
    P3 --> P4["🏗️ Phase 3 : Architecte bis (Roadmap & Issues)"]
    P4 --> P5["✅ Phase 4 : Environnement de Validation & Tests"]
    P5 --> P6["📄 Phase 5 : Walkthrough de Validation"]
```
...
→ **Créer les issues GitHub** correspondantes avec le format standard (Contexte, Fichiers, Objectifs).
...
- **Expérience / Recherche** : Exécuter les pipelines ou récupérer les résultats existants (métriques, graphiques, tableaux).
- **Tests** : Exécuter la suite de tests complète et rapporter les résultats dans le walkthrough.
- **Bibliothèque** : Vérifier que les imports fonctionnent, exécuter les tests rapides.
...
[Output complet des tests]

### Résultats d'Expériences (si applicable)
[Métriques collectées, tableaux de résultats]
...
## 🛡️ Comment le Principe Fail-Fast Est-il Garanti ?

| Erreur Rencontrée | Action Immédiate |
| :--- | :--- |
...
| **Aucune PR avec le label `valid`** | Informer et STOP (arrêt propre, pas de crash) |
