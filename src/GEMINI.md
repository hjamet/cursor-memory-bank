...
| **Obligation de Lecture Bonnes Pratiques** | Tout agent recevant une consigne d'écriture ou de modification de code (scripts, outils MCP, pipelines, modules) DOIT impérativement lire C:\Users\Jamet\Documents\VoiceNotes\_agents\skills\bonnes-pratiques-developpement\SKILL.md via view_file avant d'écrire la moindre ligne de code. |
...
|---|---|
| **Maintien permanent & Zéro Section** | Maintenir en continu `<appDataDir>/brain/<id>/task.md` affiché sur le volet latéral. **Strictement aucun titre `#` ni section Markdown**. Structure obligatoire à **2 niveaux d'indentation stricts** :<br/>1. **Niveau 1 (Chantier)** : `- [ ] / - [x] <EmojiÉvocateur> Chantier [Nom du Pilier]` (représente un pilier d'action majeur ou volet distinct du projet, et NON une micro-étape séquentielle. Statut porté par `- [ ]` ou `- [x]`).<br/>2. **Niveau 2 (Tâches)** : `  - [ ] /   - [/] /   - [x] [Action concrète ou livrable]` (indenté de 2 espaces sous son chantier). |
| **Emojis Distinctifs Sémantiques (Zéro Porte-Clés Uniforme)** | Chaque chantier de Niveau 1 s'ouvre obligatoirement par un émoji sémantique propre et évocateur de sa nature (`📝 Rédaction`, `🔍 Investigation`, `🏛️ Administratif`, `🧪 Expérimentation`, `🧠 Apprentissage`, `📦 Livraison`). **INTERDICTION FORMELLE** d'utiliser un émoji générique répétitif (ex: le porte-clés `🏷️`) sur l'ensemble des chantiers. |
| **Granularité & Périmètre des Chantiers** | Un chantier de Niveau 1 regroupe obligatoirement l'ensemble des sous-tâches concourant au même livrable ou volet d'action. Interdiction de scinder un flux unifié (cadrage, rédaction, correction, compilation) en chantiers distincts éphémères. |
...
| **Association Universelle d'Artéfacts (100% des Tâches)** | **CHAQUE tâche de `task.md` (qu'elle soit terminée `- [x]`, en cours `- [/]` ou planifiée `- [ ]`) comporte OBLIGATOIREMENT un lien cliquable `[Nom](file:///...)` vers son artéfact associé** : transcript de sous-agent, plan d'implémentation, diff interactif CAS, walkthrough, rapport d'exploration ou note cible du coffre. Zéro tâche orpheline de lien. |
...
| **Souveraineté Inviolable des Tâches 👤 [Henri] (Zéro Auto-Coche)** | Les tâches préfixées par `👤 [Henri]` relèvent **exclusivement de l'arbitrage et de l'action humaine**. L'agent a l'**INTERDICTION FORMELLE ET ABSOLUE de cocher une tâche `👤 [Henri]` (`- [x]`) de sa propre initiative**. Une tâche utilisateur ne peut basculer à `- [x]` que si Henri confirme explicitement dans le chat qu'il a pris connaissance de l'artéfact et réalisé/validé l'action. Tant que cette confirmation textuelle n'est pas exprimée, la tâche reste rigoureusement à l'état planifié `- [ ]`. |
| **Feux Tricolores de Dépendance Utilisateur (🔴 / 🟡 / 🟢)** | Indiquer rigoureusement l'état des dépendances de chaque tâche Henri via l'émoji d'état :<br/>- `🔴` : Tâche bloquée (dépendances amont non accomplies - ne pas toucher).<br/>- `🟡` : Travail préparatoire autorisé, mais interdiction de soumettre ou envoyer (dépendances en cours).<br/>- `🟢` : Feu vert total (toutes les dépendances amont sont achevées - prêt à exécuter/envoyer).<br/><br/>**🚫 INVARIANT STRICT D'ANTICIPATION INTERDITE (ZÉRO FEU VERT PRÉMATURÉ)** :<br/>- **INTERDICTION FORMELLE ET ABSOLUE d'assigner l'émoji `🟢` à une tâche `👤 [Henri]` tant que 100% de ses dépendances amont (recherche de fichiers, extraction documentaire, rédaction de brouillon, exécution de sous-agent, vérification de liens) ne sont pas achevées et cochées à `- [x]`.**<br/>- Toute tâche Henri associée à une action agent en cours (`- [/]`) ou planifiée (`- [ ]`) doit être STRICTEMENT typée en `🔴` (si bloquée sans le livrable) ou `🟡` (si une préparation partielle sans soumission est possible). Le basculement vers `🟢` s'opère EXCLUSIVEMENT au moment où le livrable final est délivré et lié. |
| **Immédiateté Absolue (Turn 0)** | Dès qu'une tâche est mentionnée par Henri, attribuée à Henri ou créée par l'échange, l'actualisation de `task.md` via `write_to_file` doit être le **tout premier appel d'outil du tour**, avant toute autre action. L'agent DOIT obligatoirement insérer le lien cliquable `[task.md](file:///...)` en première ligne de sa réponse textuelle dès que `task.md` est modifié, avec mention explicite des chantiers modifiés. |
...
| **Sous-Agent d'Identification & Exploration à Tour 0** | À chaque début de conversation, SANS AUCUNE EXCEPTION (qu'il s'agisse d'un projet métier, d'une tâche méta, d'une modification de code, d'un skill, d'une configuration ou d'une question rapide), l'agent racine déploie STRICTEMENT à Tour 0 le sous-agent d'identification, de briefing et d'exploration contextuelle. Zéro exemption de contexte. L'exploration périphérique (e-mails Spark, sous-notes, mémoire AIVC) ne doit JAMAIS être court-circuitée par une lecture directe unilatérale du superviseur racine. Si la session porte sur l'infrastructure ou les skills, le sous-agent identifie ou crée la note maîtresse correspondante pour enclencher le Pomodoro. |
