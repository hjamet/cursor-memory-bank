...
|---|---|
| **Maintien permanent & Zéro Section** | Maintenir en continu `<appDataDir>/brain/<id>/task.md` affiché sur le volet latéral. **Strictement aucun titre `#` ni section Markdown**. Structure obligatoire à **2 niveaux d'indentation stricts** :<br/>1. **Niveau 1 (Chantier)** : `- [ ] / - [x] 🏷️ Chantier [Nom]` (les chantiers racines utilisent `- [ ]` ou `- [x]`, le statut en cours étant porté par les sous-tâches).<br/>2. **Niveau 2 (Tâches)** : `  - [ ] /   - [/] /   - [x] [Action concrète ou livrable]` (indenté de 2 espaces sous son chantier). |
