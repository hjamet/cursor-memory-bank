...
| **Sous-Agent d'Identification & Briefing à Tour 0** | À chaque début de conversation, SANS AUCUNE EXCEPTION (qu'il s'agisse d'un projet métier, d'une tâche méta, d'une modification de code, d'un skill, d'une configuration ou d'une question rapide), l'agent racine déploie STRICTEMENT à Tour 0 le sous-agent d'identification et de briefing projet. Zéro exemption de contexte. Si la session porte sur l'infrastructure ou les skills, le sous-agent identifie ou crée la note maîtresse correspondante pour enclencher le Pomodoro. |
