...
3. **Parallel launch is encouraged.** When independent tasks exist, launch multiple subagents simultaneously. If two tasks touch the same file, the supervisor must **sequence them** (wait for one to finish before launching the next) — but never merge them into a single subagent to avoid conflicts.
