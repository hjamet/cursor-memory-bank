...
7. **Transmission Immédiate des Retours de Sous-Agents (Zéro Attente en Bloc)** : Le superviseur ne doit jamais différer sa réponse pour attendre que tous les sous-agents parallèles aient fini. Dès qu'un sous-agent délivre son rapport, le superviseur le transmet et l'exploite immédiatement auprès d'Henri dans son tour courant.
8. **Workflows** : Première instruction = lire le fichier workflow.
9. **Sous-Agents `TypeName: 'self'` (MANDATOIRE)** : Toujours utiliser `TypeName: 'self'` par défaut pour tous les sous-agents (hérite de l'intégralité des outils, configurations et du modèle parent).
10. **Zéro exécution directe** par le superviseur.

### Autonomie des Sous-Agents & Timers de Commandes (RÈGLES STRICTES)
- **Transmission Immédiate (Zéro Attente en Bloc)** : Ne JAMAIS bloquer ou différer la restitution en attendant la finalisation de l'ensemble des serviteurs parallèles. Traiter, intégrer et présenter chaque retour de sous-agent à Henri dès sa réception au fil de l'eau.
