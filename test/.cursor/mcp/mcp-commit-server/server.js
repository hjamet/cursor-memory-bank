...
async function ensureLogsDirExists() {
    if (!fs.existsSync(LOGS_DIR)) {
        try {
            ...
            // console.error(`[ToolsMCP Setup] Error creating logs directory ${LOGS_DIR}:`, err); // Commented to prevent JSON-RPC pollution
        }
    }
}
...
const server = new McpServer({
    name: "ToolsMCP",
    version: "0.7.0", // Incremented version for new tool
    capabilities: {
        tools: {
...
// It remains here for backward compatibility but will be removed in a future version.
/*
server.tool(
    'regex_edit',
    {
        target_file: z.string().describe("The relative path to the file to be modified."),
        regex_pattern: z.string().describe("The JavaScript regex pattern to find the content to replace. The pattern should NOT include the '/g' flag; global replacement is assumed."),
        replacement_text: z.string().describe("The text to replace the matched content with. Can include backreferences like $1, $2, etc."),
        on_edit_fail: z.enum(['fail', 'return_original']).optional().default('fail').describe("Behavior if the regex pattern does not match anything in the file. 'fail' returns an error; 'return_original' returns the unmodified file content."),
    },
    handleRegexEdit
);
*/
...
async function startServer() {
    // console.log('[MCP Server] Initializing ToolsMCP server...'); // Commented to prevent JSON-RPC pollution
